...
public interface IUserRepo extends CrudRepository<User, Long> {
    ...
    @Query(value = "select user.id, user.address, user.avatar, user.dateOfBirthday, user.email, user.password, user.phone, user.repass, user.username, user.role_id, user.fullname from case_study_4.user  inner join friend on friend.id_friend_id = user.id where friend.id_user_id = :user and friend.status = 1;", nativeQuery = true)
